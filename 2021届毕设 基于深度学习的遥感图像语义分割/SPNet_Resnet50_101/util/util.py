...
def intersectionAndUnionGPU(output, target, K, ignore_index=255):
    # 'K' classes, output and target sizes are N or N * L or N * H * W, each value in range 0 to K - 1.
    assert (output.dim() in [1, 2, 3]) # dim()会返回有几个维度
    assert output.shape == target.shape # shape返回 B*H*W
    output = output.view(-1) # 展平
    target = target.view(-1)
    output[target == ignore_index] = ignore_index # 将output中的ignore_index换成255
    intersection = output[output == target] # 预测正确的像素，是一个长度为预测正确像素数量的列表
