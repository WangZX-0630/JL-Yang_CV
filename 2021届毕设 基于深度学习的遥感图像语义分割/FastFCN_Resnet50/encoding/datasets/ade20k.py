...
class ADE20KSegmentation(BaseDataset):
    ...
    def __init__(self, root=os.path.expanduser('~/.encoding/data'), split='train',
        ...
        # root=~/.encoding/data/ADEChallengeData2016
...
def _get_ade20k_pairs(folder, split='train'):
    # folder=~/.encoding/data/ADEChallengeData2016
    def get_path_pairs(img_folder, mask_folder):
        img_paths = [] #空列表
        mask_paths = []
        for filename in os.listdir(img_folder):
            basename, _ = os.path.splitext(filename)
            # splitext分离文件名与拓展名，返回第一个元素文件名，第二个元素拓展名（如.jpg)
            if filename.endswith(".jpg"):
                imgpath = os.path.join(img_folder, filename)
                maskname = basename + '.png' # gt是png文件
