...
class PostdamSegmentation(BaseDataset):
    BASE_DIR = 'postdam1024'
    NUM_CLASS = 6
    def __init__(self, root=os.path.expanduser('/f2020/shengyifan/data'), split='train',
                 mode=None, transform=None, target_transform=None, **kwargs):
        super(PostdamSegmentation, self).__init__(
            root, split, mode, transform, target_transform, **kwargs)
        # assert exists and prepare dataset automatically
        root = os.path.join(root, self.BASE_DIR)
        # root=/f2020/shengyifan/data/postdam
        assert os.path.exists(root), "Please setup the dataset using" + \
            "encoding/scripts/prepare_ade20k.py"
        self.images, self.masks = _get_postdam_pairs(root, split)
        if split != 'test':
...
def _get_postdam_pairs(folder, split='train'):
    # folder=/f2020/shengyifan/data/postdam
    def get_path_pairs(img_folder, mask_folder):
        # img_folder=/f2020/shengyifan/data/postdam/train/img
        img_paths = []
        mask_paths = []
        for filename in os.listdir(img_folder):
            basename, _ = os.path.splitext(filename)
            if filename.endswith(".png"):
                # filename=top_potsdam_6_9_rgb-21.bmp
                # basename=top_potsdam_6_9_rgb-21
                imgpath = os.path.join(img_folder, filename)
                maskname = basename + '_label' + '.png'
    ...
    if split == 'train':
        # folder=/f2020/shengyifan/data/postdam
        img_folder = os.path.join(folder, 'train/img')
        mask_folder = os.path.join(folder, 'train/gtblack')
        img_paths, mask_paths = get_path_pairs(img_folder, mask_folder)
        # print('len(img_paths):', len(img_paths))
        # assert len(img_paths) == 20210
    elif split == 'val':
        img_folder = os.path.join(folder, 'val/img')
        mask_folder = os.path.join(folder, 'val/gtblack')
        img_paths, mask_paths = get_path_pairs(img_folder, mask_folder)
