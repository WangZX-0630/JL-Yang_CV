...
from Training.ptsemseg.models import get_model
from Training.ptsemseg.loss import get_loss_function
from Training.ptsemseg.loader import get_loader
from Training.ptsemseg.utils import get_logger
from Training.ptsemseg.metrics import runningScore, averageMeter
from Training.ptsemseg.augmentations import get_composed_augmentations
from Training.ptsemseg.optimizers import get_optimizer
from Training.ptsemseg.utils import convert_state_dict, clean_state_dict
import pdb
...
def train(cfg, logger, logdir):
    ...
    while cnt_iter <= cfg["training"]["train_iters"]:
        for (f_img, labels) in trainloader:
            ...
            if (cnt_iter + 1) % cfg["training"]["val_interval"] == 0 or (cnt_iter + 1) == cfg["training"][
                "train_iters"]:
                model.eval()
                with torch.no_grad():
