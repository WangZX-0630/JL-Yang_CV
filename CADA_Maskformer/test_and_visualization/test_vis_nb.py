...
if __name__ == "__main__":
    ...
    if True:
        ...
        path20 = '/root/Mask/datasets/V_test/val_nb_label/'#val_label_png文件夹
        rootdir1=sorted(os.listdir(path10))
        rootdir2=sorted(os.listdir(path20))
        print(len(rootdir1))
        print(len(rootdir2))
        save_path = "/root/Mask/res/DC_V_0000_4"#精度计算结果保存文件夹
        RGB_path = "/root/Mask/res/DC_V_0000_4/png_nb/"#可视化结果保存文件夹
        save_name = "DC_V_0000_nb"
        
        path1= path10 + rootdir1[0]
        path2= path20 + rootdir1[0].split('.tif')[0] + '_noBoundary.png'
        print(path1)
        print(path2)
...
#         imgf[:,:,:]=[0,0,255] 
#         #imgf[lab == 255] = [0,255,0]
#         imgf[lab == visualized_output] = [0,255,0]
#         rgb_path = RGB_path + rootdir1[0].split('.')[0] + '.jpg'
#         cv2.imwrite(rgb_path, imgf)
        
        confu_mat_total = cal_confu_matrix(lab,visualized_output, 6)
        for i in range(1,len(rootdir1)):
            path1= path10 + rootdir1[i]
            path2= path20 + rootdir1[i].split('.tif')[0] + '_noBoundary.png'
            img = read_image(path1, format="BGR")
            print(path1)
            print(path2)
...
#             #imgf[lab == 255] = [0,255,0]
#             imgf[lab == visualized_output] = [0,255,0]
#             rgb_path = RGB_path + rootdir1[i].split('.')[0] + '.jpg'
#             cv2.imwrite(rgb_path, imgf)
