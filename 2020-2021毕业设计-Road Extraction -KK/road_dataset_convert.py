...
class RoadDataset(data.Dataset):
    def __init__(
        self, config, dataset_name, seed=7, multi_scale_pred=False, is_train=True
    ):
        # Seed
        np.random.seed(seed)
...
class DeepGlobeDataset(RoadDataset):
    def __init__(self, config, seed=7, multi_scale_pred=False, is_train=True):
...
#         image, gt = self.getRoadData(index)
        ...
        for i, val in enumerate(scale):
            ...
            linestrings = affinity_utils.getKeypoints(
...
#         erased_gt = torch.from_numpy(erased_gt)
        ...
        image, gt, erased_gt = self.getPredData(index)
        c, h, w = image.shape
        gt /= 255.0
        erased_gt /= 255.0
