...
from road_dataset_test import DeepGlobeDatasetCorrupt, SpacenetDatasetCorrupt
...
#         best_miou = miou
#         util.save_checkpoint(epoch, train_loss_iou / len(train_loader), model, optimizer, best_accuracy, best_miou, config, experiment_dir)

    return train_loss_iou / len(train_loader)
...
def test(epoch):
    ...
    for i, data in enumerate(val_loader, 0):
        inputs, labels, erased_label, image_name = data
        batch_size = inputs.size(0)
#         print(i)
        print(image_name)
...
#         _, predicted = torch.max(outputs.data, 1)
        ...
        if i % 1 == 0:
            images_path = "{}/results/".format(experiment_dir)
...
#             cv2.imwrite(image_name[0][:-4]+'_mask'+image_name[0][-4:], predicted_.reshape(b * h, w, 3))
