...
class RoadDataset(data.Dataset):
    def __init__(
        self, config, dataset_name, seed=7, multi_scale_pred=True, is_train=False
    ):
        # Seed
        np.random.seed(seed)
...
#         self.dir_pred = '/root/data/deepglobe/pred_dlink/' if is_train else '/root/data/deepglobe/pred_val_dlink/'
        self.dir_pred = '/root/data/deepglobe/pred_dlink2/' if is_train else '/root/data/deepglobe/pred_val_dlink2/'

    
        self.img_root = os.path.join(self.dir, "images/")
        self.gt_root = os.path.join(self.dir, "gt/")
        self.img_root_pred = os.path.join(self.dir_pred, "images/")
        self.gt_root_pred = os.path.join(self.dir_pred, "gt/")
        ...
        self.files_pred = collections.defaultdict(list)
        ...
        for f in self.images:
            self.files_pred[self.split].append(
                {
                    "img": self.img_root_pred
                    + f
                    + self.config[dataset_name]["image_suffix"],
                    "lbl": self.gt_root_pred + f.replace('_sat_', '_mask_') + self.config[dataset_name]["gt_suffix"],
                }
            )
...
#             image = cv2.imread(image_dict["img"]).astype(np.float)
        if os.path.isfile(image_dict["img"][:-4]+'_sat'+image_dict["img"][-4:]):
            image = cv2.imread(image_dict["img"][:-4]+'_sat'+image_dict["img"][-4:]).astype(np.float)
        else:
...
#             gt = cv2.imread(image_dict["lbl"], 0).astype(np.float)
        if os.path.isfile(image_dict["lbl"][:-4]+'_mask'+image_dict["lbl"][-4:]):
            gt = cv2.imread(image_dict["lbl"][:-4]+'_mask'+image_dict["lbl"][-4:], 0).astype(np.float)
        else:
        ...
        ### 
        image_name = image_dict["lbl"]
        return image, gt, image_name
#         return image, gt

    def getPredData(self, index):

        image_dict = self.files[self.split][index]
        image_dict_pred = self.files_pred[self.split][index]
        # read each image in list
        
#         if os.path.isfile(image_dict["img"][:-4]):
#             image = cv2.imread(image_dict["img"][:-4]).astype(np.float)
        if os.path.isfile(image_dict["img"][:-4]+'_sat'+image_dict["img"][-4:]):
            image = cv2.imread(image_dict["img"][:-4]+'_sat'+image_dict["img"][-4:]).astype(np.float)
        else:
            print("ERROR: couldn't find image -> ", image_dict["img"])

#         if os.path.isfile(image_dict["lbl"][:-4]):
#             gt = cv2.imread(image_dict["lbl"][:-4], 0).astype(np.float)
        if os.path.isfile(image_dict["lbl"][:-4]+'_mask'+image_dict["lbl"][-4:]):
            gt = cv2.imread(image_dict["lbl"][:-4]+'_mask'+image_dict["lbl"][-4:], 0).astype(np.float)
        else:
            print("ERROR: couldn't find image -> ", image_dict["lbl"])
            
        if os.path.isfile(image_dict_pred["lbl"][:-4]+'_mask'+image_dict_pred["lbl"][-4:]):
            pred = cv2.imread(image_dict_pred["lbl"][:-4]+'_mask'+image_dict_pred["lbl"][-4:], 0).astype(np.float)
        else:
            print("ERROR: couldn't find image -> ", image_dict_pred["lbl"])

        if self.split == "train":
            image, gt , pred= self.random_crop_pred(image, gt, pred, self.crop_size)
        else:
            image = cv2.resize(
                image,
                (self.crop_size[0], self.crop_size[1]),
        ...
        if self.augmentation == 1:
            ...
            image = np.ascontiguousarray(image[:, ::flip, :])
            gt = np.ascontiguousarray(gt[:, ::flip])
            pred = np.ascontiguousarray(gt[:, ::flip])
            rotation = np.random.randint(4) * 90
            M = cv2.getRotationMatrix2D((w / 2, h / 2), rotation, 1)
            image = cv2.warpAffine(image, M, (w, h))
            gt = cv2.warpAffine(gt, M, (w, h))
            pred = cv2.warpAffine(gt, M, (w, h))

        image = self.reshape(image)
        image = torch.from_numpy(np.array(image))
        ### 
        image_name = image_dict["lbl"]
        return image, gt, pred, image_name
    ...
    def random_crop_pred(self, image, gt, pred, size):
        ...
        image = image[start_x: start_x + crop_w, start_y: start_y + crop_h, :]
        gt = gt[start_x: start_x + crop_w, start_y: start_y + crop_h]
        pred = pred[start_x: start_x + crop_w, start_y: start_y + crop_h]
...
class DeepGlobeDataset(RoadDataset):
    ...
    def __getitem__(self, index):
        ### gai
        image, gt, image_name = self.getRoadData(index)
#         image, gt = self.getRoadData(index)
        c, h, w = image.shape
        ...
        return image, labels, vecmap_angles, image_name
#         return image, labels, vecmap_angles, 
...
#         erased_gt = torch.from_numpy(erased_gt)
        ...
        image, gt, erased_gt, image_name = self.getPredData(index)
        c, h, w = image.shape
        gt /= 255.0
        erased_gt /= 255.0

        return image, [gt], [erased_gt], image_name
