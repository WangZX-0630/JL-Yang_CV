...
def eval_function():
    label_path = 'submits/label/'
    predict_path = 'submits/img/'
    ...
    for im in pres:
        if im[-4:] == '.png':
            ...
            label = cv2.imdecode(np.fromfile(lab_path,dtype=np.uint8),0)
            pre = cv2.imdecode(np.fromfile(pre_path,dtype=np.uint8),0)
            label[label>0] = 1
            pre[pre>0] = 1
            labels.append(label)
            predicts.append(pre)
    ...
    for im in pres:
        if im[-4:] == '.png':
            ...
            pre_path = os.path.join(predict_path, im)
            lb = cv2.imdecode(np.fromfile(lb_path,dtype=np.uint8),0)
            pre = cv2.imdecode(np.fromfile(pre_path,dtype=np.uint8),0)
            lb[lb>0] = 1
            pre[pre>0] = 1
            lb = lb.flatten()
    ...
    print('class_precision: ', precision)
