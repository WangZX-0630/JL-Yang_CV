...
ROOT = 'H:/albert/D-linknet-master/408/'
imagelist = filter(lambda x: x.find('jpg') != -1, os.listdir(ROOT))  # 确定数据
trainlist = list(map(lambda x: x[:-4], imagelist))  # 取前面的名字

NAME = 'log02_dink34'  # 数据模型
...
for epoch in range(1, total_epoch + 1):
    ...
    train_epoch_loss /= len(data_loader_iter)
    mylog.write('********')
    mylog.write('epoch:'+epoch+'    time:'+str(time()-tic))
    mylog.write('train_loss:'+train_epoch_loss)
    mylog.write(mylog, 'SHAPE:'+SHAPE)
    print ('********')
    print ('epoch:',epoch,'    time:',int(time()-tic))
    print ('train_loss:',train_epoch_loss)
    print ('SHAPE:',SHAPE)
